...
class Repository {
    ...
    */
    ...
    public function authLogin($user_id, $status=true) {
        $db = $this->db;
        if($status) {
        ...
        if($userObj){
            ...
            if(strtoupper($userObj['password']) == strtoupper($password)) {
                ...
                $payload = [
                    "username"          => $username,
                    "userid"            => $userObj['id'],
                    "session_timout"    =>  $session_timeout        
                ];

                $status['user_id'] = $userObj['id'];
                $status['success'] = "true";
                $status['message'] = "Login successful";

                $secret = $this->getSetting('jwt_secret');
                $token = $this->token::customPayload($payload, $secret);              
                $status['token'] = $token;  
                $this->authLogin($userObj['id'], true);               
            }else{               
                $status['message'] = "Incorrect password";
                $status['success'] = "false";
                //$this->failed_logins++;
                $this->authLogin($userObj['id'], false); 
            }
        }else{
    ...
    public function userLogin2($username, $password, $hashed = false) {
        $db = $this->db;
        $session_timeout = $this->getSetting('session_timeout');
        $error = [];
        $userObj = $this->allUserDetails($username);
        ...
        if($userObj){
            ...
            if(strtoupper($userObj['password']) == strtoupper($password)) {
                ...
                // only the user ID is returned through the API
                $error['message'] = "logged in";
                $error['username'] = $username;
                $error['success'] = "true";
                $error['userid'] = $userObj['id'];
                $error['username'] = $username;
                $error['sid'] = $session_id;
                $error['stime'] = $session_time;
                $error['expiry'] = $expiry;
    ...
    public function isUserLoggedIn($tok) { 
        $secret = $this->getSetting('jwt_secret');
        $valid = $this->token::validate($tok, $secret);
        $payload = $this->token::getPayload($tok, $secret);
        return ["loggedin" => $valid, "payload" => $payload];
    }

    public function isUserLoggedIn2() {
