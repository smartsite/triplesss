...
//use Triplesss\text\Emoji as Emoji; not implemented
...
class Repository {
    ...
    Public function editPost(Post $post, String $text, Image $image) {
        ...
        /**
         *  TODO: do an update on the text in the text table if it's not empty,
    ...
    Public function getFeedPosts(Feed $feed, Int $visibility = 0) {
        ...
        if($filter) {
            ...
            if($filter->type == 'Orange') { 
                $range = $filter->getRange();
                if(is_array($range)) {                    
                    $start = $range[0];
                    $rows = $range[1] - $range[0];
                    $s.= ' ORDER BY date DESC  LIMIT '.$start.', '.$rows;
                }               
            }
            ...
            if($filter->type == 'range') { 
                ...
                if(is_array($range)) {                    
                    ...
                    $rows = $range[1] - $range[0];
                    $s = 'SELECT feed_post.post_id FROM feed_post JOIN post ON post.post_id = feed_post.post_id 
                         WHERE feed_id='.$feed->id.' ORDER BY post.id DESC LIMIT '.$start.', '.$rows;
                }               
            }            
        }
    ...
    Public function getAggregatedPosts(Aggregator $aggregator) {
        ...
        //  fetch all post ids for connected users 
        if($userid > 1) {
    ...
    public function userLogin($username, $password, $hashed = false) {
        ...
        if($userObj){
            if($hashed) {
                // TODO: $password = hash ("sha256", $password);
    ...
    public function removeConnection(User $from, User $to, Connection $connection) {
        /**
         *  alias for addConnection type=0 
         */     
    }
